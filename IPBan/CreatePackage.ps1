...
& "c:/program files/dotnet/dotnet.exe" publish IPBan.csproj -f $netFrameworkVersion -o package/linux-x64 -c Release -r linux-x64 /p:PublishTrimmed=$publishTrimmed /p:PublishSingleFile=true /p:IncludeNativeLibrariesForSelfExtract=true; CheckFail

# IPBan Windows x64
& "c:/program files/dotnet/dotnet.exe" restore -r win-x64; CheckFail
& "c:/program files/dotnet/dotnet.exe" clean -c Release; CheckFail
& "c:/program files/dotnet/dotnet.exe" publish IPBan.csproj -f $netFrameworkVersion -o package/win-x64 -c Release -r win-x64 /p:PublishTrimmed=$publishTrimmed /p:PublishSingleFile=true /p:IncludeNativeLibrariesForSelfExtract=true; CheckFail
...
& "c:/program files (x86)/dotnet/dotnet.exe" publish IPBan.csproj -f $netFrameworkVersion -o package/win-x86 -c Release -r win-x86 /p:PublishTrimmed=$publishTrimmed /p:PublishSingleFile=true /p:IncludeNativeLibrariesForSelfExtract=true; CheckFail
